...
ggsave('kmeans_sse_ath.pdf')
ggsave('kmeans_sse_ath.jpg')
...
ggsave(paste0(prefix, '_genes_ath.pdf'), width = 10, dpi = 320)
ggsave(paste0(prefix, '_genes_ath.jpg'), width = 10, dpi = 320)
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
ggplot(traitCorPlot, aes(x = x, y = y, fill = correlation)) +
  ...
  xlab('Trait') +
  ylab('Cluster')
ggsave(paste0(prefix, '_trait_ath.jpg'))
ggsave(paste0(prefix, '_trait_ath.pdf'))
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
heatRawPlot <- heatPlot %>%
  ...
  gather(sample, raw, -1) %>%
  mutate(x = rep(0 : 15, each = nrow(heatPlot))) %>%
  mutate(y = rep(0 : (nrow(heatPlot) - 1), 16))
...
sigte <- heatGroupPlot %>%
  ...
  mutate(x = rep(c(0.2, 0, 0.4), each = max(cl))) %>%
...
ggsave(file = paste0(prefix, '_heatmap_merge_ath.pdf'), plot = g, width= 10)
ggsave(file = paste0(prefix, '_heatmap_merge_ath.jpg'), plot = g, width = 10)
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
